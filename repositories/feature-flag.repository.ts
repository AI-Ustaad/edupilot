...
import { IFeatureFlagRepository, FeatureFlags } from "@/interfaces/IFeatureFlagRepository";

export class FeatureFlagRepository implements IFeatureFlagRepository {
  private collection = "tenantFeatures";

  async findByTenant(tenantId: string): Promise<FeatureFlags | null> {
    const doc = await adminDb.collection(this.collection).doc(tenantId).get();
    if (!doc.exists) return null;
    return { id: doc.id, ...doc.data() } as FeatureFlags;
  }
