...
const Navbar = () => {
  ...
  return (
    <motion.nav initial={{ y: -100 }} animate={{ y: 0 }} transition={{ duration: 0.5 }} className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "glass-card !rounded-none !bg-white/70 backdrop-blur-xl" : "bg-transparent"}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center gap-8">
            {navItems.map((item) => (
              <a key={item} href={`#${item.toLowerCase()}`} className="text-sm font-medium text-slate-600 hover:text-primary transition-colors">{item}</a>
            ))}
            <Link href="/login" className="text-sm font-medium text-slate-600 hover:text-primary">Log in</Link>
          ...
          <div className="flex items-center gap-2 md:hidden">
            <button onClick={() => setIsOpen(!isOpen)} className="p-2 rounded-lg glass-card">{isOpen ? <X size={20} className="text-slate-700" /> : <Menu size={20} className="text-slate-700" />}</button>
          </div>
        </div>
      </div>
      <AnimatePresence>
...
const HeroSection = () => {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20">
      {/* Floating icons (small decorative) */}
      ...
      <motion.div style={{ opacity, scale }} className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        >
          ...
          <span className="text-xs font-medium text-slate-600">AI-Powered • Cloud-Based • Enterprise SaaS</span>
        </motion.div>
        ...
        <motion.p
          ...
          className="mt-6 text-lg sm:text-xl text-slate-500 max-w-3xl mx-auto"
        >
...
const TrustStats = () => {
  ...
  return (
    <section ref={ref} className="py-16 glass-card !rounded-none !bg-white/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <section id="features" className="py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <motion.p variants={fadeUp} className="mt-4 text-lg text-slate-500 max-w-3xl mx-auto">
...
const AIIntelligence = () => {
  return (
    <section className="py-24 bg-gradient-to-br from-primary/10 via-accent/10 to-info/10 relative overflow-hidden">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 glass-card !rounded-full px-4 py-1.5 mb-6">
              ...
              <span className="text-xs font-medium text-slate-600">AI-Powered Intelligence</span>
            </div>
...
const PricingSection = () => {
  ...
  if (!mounted) return <section id="pricing" className="py-24"><div className="text-center text-slate-500">Loading...</div></section>;
          ...
          {plans.map((plan, idx) => (
            <motion.div key={idx} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} whileHover={{ y: -8 }} className={`relative glass-card p-8 ${plan.popular ? "!border-primary ring-2 ring-primary/20" : ""}`}>
              ...
              <div className="mt-4 flex items-baseline gap-1">
                {plan.isCustom ? <span className="text-4xl font-black text-slate-800">Custom</span> : <span className="text-4xl font-black text-slate-800">{formatPrice(plan.usdPrice, currency)}</span>}
                {!plan.isCustom && plan.usdPrice > 0 && <span className="text-slate-500">/month</span>}
              </div>
              <ul className="mt-6 space-y-3">
                {plan.features.map((feature, i) => (
                  <li key={i} className="flex items-center gap-2 text-sm text-slate-600"><CheckCircle size={16} className="text-success" />{feature}</li>
                ))}
              </ul>
...
const Footer = () => (
  <footer className="glass-card !rounded-none !bg-white/30 border-t border-slate-200 py-12">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
        ...
        <div><h4 className="font-bold text-slate-800 mb-4">Product</h4><ul className="space-y-2 text-sm text-slate-500"><li><a href="#features">Features</a></li><li><a href="#pricing">Pricing</a></li></ul></div>
        <div><h4 className="font-bold text-slate-800 mb-4">Resources</h4><ul className="space-y-2 text-sm text-slate-500"><li><a href="#">Documentation</a></li><li><a href="#">API</a></li></ul></div>
        <div><h4 className="font-bold text-slate-800 mb-4">Legal</h4><ul className="space-y-2 text-sm text-slate-500"><li><a href="#">Privacy</a></li><li><a href="#">Terms</a></li></ul></div>
      </div>
      <div className="mt-12 pt-8 border-t border-slate-200 text-center text-sm text-slate-400"><p>&copy; 2025 EduPilot SaaS. All rights reserved.</p></div>
    </div>
  </footer>
);
...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#f8fafc] via-[#f1f5f9] to-[#e2e8f0] relative">
      {/* Background animation (educational floating icons) */}
      <BackgroundAnimation />
