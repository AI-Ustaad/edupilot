  ...
  return (
    <div className="min-h-screen bg-[#F8F9FE] text-[#302B52] font-sans">
      <nav className="flex justify-between items-center px-10 py-6 bg-white shadow-sm">
        ...
        <Link href="/login">
          <button className="bg-[#302B52] text-white px-8 py-3 rounded-xl font-bold hover:bg-[#7166F9] transition-all">Login</button>
        </Link>
      </nav>
      ...
      <section className="px-10 py-24 flex flex-col items-center text-center">
        ...
        <p className="text-xl text-gray-500 mb-12 max-w-2xl">Pakistan's most advanced AI-powered school management system.</p>
        <Link href="/login">
      ...
      <section className="px-10 pb-24 grid grid-cols-1 md:grid-cols-4 gap-6 max-w-7xl mx-auto">
        {[{t:"Students", i:<Users/>, c:"bg-purple-50"}, {t:"Attendance", i:<ClipboardCheck/>, c:"bg-green-50"}, {t:"Fees", i:<CreditCard/>, c:"bg-yellow-50"}, {t:"AI Results", i:<BookOpen/>, c:"bg-blue-50"}].map((item, i) => (
          <div key={i} className="bg-white p-8 rounded-[32px] shadow-sm border border-gray-100">
            <div className={`${item.c} w-16 h-16 rounded-2xl flex items-center justify-center mb-6 text-[#302B52]`}>{item.i}</div>
            <h3 className="text-xl font-bold">{item.t}</h3>
            <p className="text-gray-400 text-sm mt-2">Professional management tools.</p>
          </div>
        ))}
      </section>
    </div>
