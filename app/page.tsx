...
const Navbar = () => {
  ...
  return (
    <motion.nav initial={{ y: -100 }} animate={{ y: 0 }} transition={{ duration: 0.5 }} className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "glass-card !rounded-none !bg-dark-800/60 backdrop-blur-xl" : "bg-transparent"}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center gap-8">
            {navItems.map((item) => (
              <a key={item} href={`#${item.toLowerCase()}`} className="text-sm font-medium text-white/70 hover:text-primary transition-colors">{item}</a>
            ))}
            <Link href="/login" className="text-sm font-medium text-white/70 hover:text-primary">Log in</Link>
          ...
          <div className="flex items-center gap-2 md:hidden">
            <button onClick={() => setIsOpen(!isOpen)} className="p-2 rounded-lg glass-card">{isOpen ? <X size={20} className="text-white" /> : <Menu size={20} className="text-white" />}</button>
          </div>
        </div>
      </div>
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="md:hidden glass-card !rounded-none">
            <div className="px-4 py-4 space-y-3">
              {navItems.map((item) => (
                <a key={item} href={`#${item.toLowerCase()}`} className="block py-2 text-white/70 font-medium" onClick={() => setIsOpen(false)}>{item}</a>
              ))}
              <Link href="/login" className="block py-2 text-white/70 font-medium" onClick={() => setIsOpen(false)}>Log in</Link>
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20">
      ...
      <motion.div style={{ opacity, scale }} className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <motion.div
          ...
          className="inline-flex items-center gap-2 glass-card !rounded-full px-4 py-1.5 mb-6 !bg-white/10 backdrop-blur-md"
        >
          <span className="relative flex h-2 w-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2 w-2 bg-green-500"></span>
          </span>
          <span className="text-xs font-medium text-white/80">AI-Powered • Cloud-Based • Enterprise SaaS</span>
        </motion.div>
...
const TrustStats = () => {
  ...
  return (
    <section ref={ref} className="py-16 glass-card !rounded-none !bg-white/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {[
            ...
            { value: counts.schools, label: "Schools", color: "text-warning" },
          ].map((stat, idx) => (
            <motion.div
              key={idx}
              initial={{ opacity: 0, y: 20 }}
    ...
    <section id="features" className="py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <motion.p variants={fadeUp} className="mt-4 text-lg text-white/60 max-w-3xl mx-auto">
...
const AIIntelligence = () => {
  return (
    <section className="py-24 bg-gradient-to-br from-dark-800/50 via-dark-900/50 to-dark-700/50 relative overflow-hidden">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 glass-card !rounded-full px-4 py-1.5 mb-6">
              ...
              <span className="text-xs font-medium text-white">AI-Powered Intelligence</span>
            </div>
          ...
          >
            <div className="glass-card p-8">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="font-bold text-white">AI Analytics Dashboard</h3>
              </div>
              <div className="space-y-4">
                <div className="flex justify-between items-center"><span className="text-white/60">Weak Student Prediction</span><span className="text-primary font-bold">85% accuracy</span></div>
                <div className="w-full bg-white/10 rounded-full h-2"><div className="bg-primary h-2 rounded-full w-[85%]" /></div>
                <div className="flex justify-between items-center"><span className="text-white/60">Attendance Forecast</span><span className="text-secondary font-bold">92% confidence</span></div>
                <div className="w-full bg-white/10 rounded-full h-2"><div className="bg-secondary h-2 rounded-full w-[92%]" /></div>
                <div className="flex justify-between items-center"><span className="text-white/60">Performance Prediction</span><span className="text-accent font-bold">78% accuracy</span></div>
                <div className="w-full bg-white/10 rounded-full h-2"><div className="bg-accent h-2 rounded-full w-[78%]" /></div>
              </div>
            </div>
          </motion.div>
        </div>
...
const PricingSection = () => {
  ...
  if (!mounted) return <section id="pricing" className="py-24"><div className="text-center text-white">Loading...</div></section>;
    ...
    <section id="pricing" className="py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="mt-4 text-lg text-white/60 max-w-2xl mx-auto">Choose the perfect plan for your institution.</p>
        </div>
        <div className="grid md:grid-cols-3 gap-8">
          {plans.map((plan, idx) => (
            <motion.div key={idx} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} whileHover={{ y: -8 }} className={`relative glass-card p-8 ${plan.popular ? "!border-primary ring-2 ring-primary/20" : ""}`}>
              ...
              <div className="mt-4 flex items-baseline gap-1">
                {plan.isCustom ? <span className="text-4xl font-black text-white">Custom</span> : <span className="text-4xl font-black text-white">{formatPrice(plan.usdPrice, currency)}</span>}
                {!plan.isCustom && plan.usdPrice > 0 && <span className="text-white/50">/month</span>}
              </div>
              <ul className="mt-6 space-y-3">
                {plan.features.map((feature, i) => (
                  <li key={i} className="flex items-center gap-2 text-sm text-white/70"><CheckCircle size={16} className="text-success" />{feature}</li>
                ))}
              </ul>
              <button onClick={loginWithGoogle} className="mt-8 block w-full text-center btn-primary">Get Started</button>
            </motion.div>
          ))}
        </div>
      </div>
...
const Footer = () => (
  <footer className="glass-card !rounded-none !bg-dark-800/30 border-t border-white/10 py-12">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
        <div>
          <div className="flex items-center gap-2 mb-4">
            <div className="w-8 h-8 bg-gradient-to-br from-primary to-accent rounded-lg flex items-center justify-center"><span className="text-white font-black text-sm">EP</span></div>
            <span className="text-lg font-black text-white">EduPilot</span>
          </div>
          <p className="text-sm text-white/50">The AI-Powered Operating System for Modern Education.</p>
        </div>
        <div><h4 className="font-bold text-white mb-4">Product</h4><ul className="space-y-2 text-sm text-white/50"><li><a href="#features">Features</a></li><li><a href="#pricing">Pricing</a></li></ul></div>
        <div><h4 className="font-bold text-white mb-4">Resources</h4><ul className="space-y-2 text-sm text-white/50"><li><a href="#">Documentation</a></li><li><a href="#">API</a></li></ul></div>
        <div><h4 className="font-bold text-white mb-4">Legal</h4><ul className="space-y-2 text-sm text-white/50"><li><a href="#">Privacy</a></li><li><a href="#">Terms</a></li></ul></div>
      </div>
      <div className="mt-12 pt-8 border-t border-white/10 text-center text-sm text-white/30"><p>&copy; 2025 EduPilot SaaS. All rights reserved.</p></div>
    </div>
  </footer>
);
