...
export default function SettingsPage() {
  ...
  // ==========================================
  // SAVE FUNCTIONS (RESTORED & MERGED)
  // ==========================================
  
  // 1. Save Identity & Financial (Global Settings)
  ...
  // 2. Add New Class/Section (Academic Structure)
  const handleAddSection = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, "sections", docId), {
        classGrade: newClassGrade, sectionName: newSectionName, createdAt: serverTimestamp()
      });
  ...
  // The Big Save All Button Header Action
  ...
  return (
    <div className="animate-fade-in space-y-6 pb-20">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        ...
        <button onClick={handleSaveAllSettings} disabled={loading} className="bg-[#3ac47d] text-white px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 hover:bg-[#2eaa6a] transition-all shadow-md disabled:opacity-50">
           {loading ? <Loader2 size={18} className="animate-spin"/> : <Save size={18}/>} Save All Settings
        </button>
      </div>

      {success && <div className="bg-green-50 text-green-700 p-4 rounded-xl flex items-center gap-3 border border-green-100 font-bold"><CheckCircle2 size={20}/> Action Completed Successfully!</div>}
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        {/* --- SETTINGS SIDEBAR (Exact Match to Uploaded Image) --- */}
        ...
        <div className="md:col-span-8 lg:col-span-9">
             ...
             <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100 animate-fade-in-up">
                ...
                <div className="bg-slate-50 border border-slate-200 p-6 rounded-2xl mb-8">
                   ...
                   <div className="flex flex-col sm:flex-row gap-4 items-end">
                      ...
                      <button onClick={handleAddSection} disabled={loading} className="bg-purple-600 text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 hover:bg-purple-700 shadow-md whitespace-nowrap">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                   {sections.length === 0 ? <p className="text-slate-400 font-bold text-sm">No sections created yet.</p> : sections.map(s => (
                      <div key={s.id} className="bg-white border border-slate-200 p-4 rounded-xl flex justify-between items-center group hover:border-purple-300">
                         <div>
                         ...
                         <button onClick={() => handleDeleteSection(s.id)} className="text-red-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={16}/></button>
                      </div>
                   ))}
                </div>
             </div>
           ...
           {/* 4. SYSTEM SECURITY (RESTORED!) */}
           {activeTab === "security" && (
             <div className="bg-white rounded-3xl p-8 shadow-sm border border-red-100 animate-fade-in-up">
                ...
                <div className="bg-red-50 text-red-700 p-6 rounded-2xl border border-red-200">
                   ...
                   <button className="bg-white text-red-600 border border-red-200 px-6 py-2 rounded-lg font-bold hover:bg-red-600 hover:text-white transition-colors text-sm">Reset Admin Password</button>
                </div>
             </div>
           )}
             ...
             <div className="space-y-6 animate-fade-in-up">
                <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                   ...
                   <div className="grid grid-cols-2 gap-4 mb-6 border-b border-slate-100 pb-6">
                      <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Target Class</label><select value={selectedClass} onChange={e => { setSelectedClass(e.target.value); setSelectedSection(""); }} className="w-full bg-slate-50 outline-none rounded-xl px-3 py-2.5 text-sm border border-slate-200 font-bold focus:border-[#3ac47d]"><option value="">Choose Class</option>{availableClasses.map(cls => <option key={cls as string} value={cls as string}>{cls as string}</option>)}</select></div>
                      <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Target Section</label><select value={selectedSection} onChange={e => setSelectedSection(e.target.value)} disabled={!selectedClass} className="w-full bg-slate-50 outline-none rounded-xl px-3 py-2.5 text-sm border border-slate-200 font-bold focus:border-[#3ac47d] disabled:opacity-50"><option value="">Choose Section</option>{availableSections.map(sec => <option key={sec.id} value={sec.sectionName}>{sec.sectionName}</option>)}</select></div>
                   </div>
                   ...
                   <div className={`transition-opacity duration-300 ${!selectedClass || !selectedSection ? 'opacity-30 pointer-events-none' : 'opacity-100'}`}>
                      <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-3 items-end">
                         <div className="space-y-1"><label className="text-[10px] font-bold text-blue-500 uppercase tracking-widest">Period</label><select value={selectedPeriod} onChange={e => setSelectedPeriod(e.target.value)} className="w-full bg-blue-50 outline-none rounded-lg px-3 py-2.5 text-xs font-bold text-blue-700"><option value="">Period</option>{schoolPeriods.filter(p => p !== "Break").map(p => <option key={p} value={p}>{p}</option>)}</select></div>
                         <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Subject</label><select value={selectedSubject} onChange={e => setSelectedSubject(e.target.value)} className="w-full bg-slate-50 outline-none rounded-lg px-3 py-2.5 text-xs font-bold border border-slate-200"><option value="">Subject</option>{availableSubjects.map(s => <option key={s} value={s}>{s}</option>)}<option value="Games">Games</option></select></div>
                         <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Teacher</label><select value={selectedTeacher} onChange={e => setSelectedTeacher(e.target.value)} className="w-full bg-slate-50 outline-none rounded-lg px-3 py-2.5 text-xs font-bold border border-slate-200"><option value="">Teacher</option>{displayStaff.map(t => <option key={t.id} value={t.id}>{t.personal?.fullName}</option>)}</select></div>
                         <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Day Range</label><div className="flex items-center gap-1"><select value={fromDay} onChange={e => setFromDay(e.target.value)} className="w-full bg-slate-50 outline-none rounded-lg px-1 py-2.5 text-[10px] font-bold border border-slate-200">{DAYS.map(d => <option key={d} value={d}>{d.substring(0,3)}</option>)}</select><span className="text-slate-400 font-bold">-</span><select value={toDay} onChange={e => setToDay(e.target.value)} className="w-full bg-slate-50 outline-none rounded-lg px-1 py-2.5 text-[10px] font-bold border border-slate-200">{DAYS.map(d => <option key={d} value={d}>{d.substring(0,3)}</option>)}</select></div></div>
                         <button onClick={handleBatchAllot} className="bg-[#3ac47d] text-white h-[42px] rounded-lg font-black text-xs hover:bg-[#2eaa6a] shadow-sm uppercase tracking-widest">Apply</button>
                      </div>
                   </div>
                </div>
                                         ...
                                         <td key={key} className="border border-slate-200 p-1.5 text-center relative group h-14 min-w-[100px]">
                                            {cell ? (
                                               <div className="flex flex-col items-center justify-center h-full bg-blue-50/50 rounded-md border border-blue-100 p-1">
                                                  ...
                                                  <button onClick={() => handleClearCell(day, period)} className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 bg-red-100 text-red-600 p-0.5 rounded text-[8px]" title="Clear"><Trash2 size={10}/></button>
                                               </div>
