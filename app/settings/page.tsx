    ...
    <div className="animate-fade-in space-y-6 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        <div className="md:col-span-8 lg:col-span-9">
           ...
           {/* 4. SYSTEM SECURITY */}
           {activeTab === "security" && (
             <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-red-100 animate-fade-in-up">
                ...
                <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                   ...
                   <div className={`transition-opacity duration-300 ${!selectedClass || !selectedSection ? 'opacity-30 pointer-events-none' : 'opacity-100'}`}>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 items-end">
                         <div className="space-y-1"><label className="text-[10px] font-bold text-blue-500 uppercase tracking-widest">Period</label><select value={selectedPeriod} onChange={e => setSelectedPeriod(e.target.value)} className="w-full bg-blue-50 outline-none rounded-lg px-3 py-3 text-xs font-bold text-blue-700"><option value="">Period</option>{schoolPeriods.filter(p => p !== "Break").map(p => <option key={p} value={p}>{p}</option>)}</select></div>
                         <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Subject</label><select value={selectedSubject} onChange={e => setSelectedSubject(e.target.value)} className="w-full bg-slate-50 outline-none rounded-lg px-3 py-3 text-xs font-bold border border-slate-200"><option value="">Subject</option>{availableSubjects.map(s => <option key={s} value={s}>{s}</option>)}<option value="Games">Games / PT</option><option value="Library">Library</option></select></div>
                         ...
                         <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Day Range</label><div className="flex items-center gap-1"><select value={fromDay} onChange={e => setFromDay(e.target.value)} className="w-full bg-slate-50 outline-none rounded-lg px-1 py-3 text-[10px] font-bold border border-slate-200">{DAYS.map(d => <option key={d} value={d}>{d.substring(0,3)}</option>)}</select><span className="text-slate-400 font-bold">-</span><select value={toDay} onChange={e => setToDay(e.target.value)} className="w-full bg-slate-50 outline-none rounded-lg px-1 py-3 text-[10px] font-bold border border-slate-200">{DAYS.map(d => <option key={d} value={d}>{d.substring(0,3)}</option>)}</select></div></div>
