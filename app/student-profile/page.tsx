...
import { ArrowLeft, Printer, User, CheckCircle2, XCircle, Clock, CalendarDays, X, ShieldCheck, Phone, BookOpen, CreditCard, Activity, Receipt } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        attSnap.forEach((doc) => {
          const data = doc.data(); aRecs.push(data);
          if (data.status === "Present") p++; if (data.status === "Absent") a++; if (data.status === "Leave") l++;
        });
        const total = p + a + l;
        ...
        feeSnap.forEach((doc) => {
          const data = doc.data(); fRecs.push(data);
          tFee += Number(data.amount || 0);
        });
        ...
        <div className="md:col-span-8 space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex items-center gap-4">
              ...
              <div><p className="text-xs text-gray-400 font-bold uppercase tracking-wider">Exams Taken</p><p className="text-2xl font-black text-[#0F172A]">0</p></div>
            </div>
          </div>
          <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 min-h-[300px]"><h3 className="text-xs font-bold text-[#3ac47d] uppercase tracking-widest mb-5 flex items-center gap-2"><Activity size={16} /> Academic Performance</h3><div className="h-full flex flex-col items-center justify-center text-slate-400 mt-10"><Activity size={48} className="mb-4 opacity-20" /><p className="font-medium text-sm">No exam records found.</p></div></div>
        </div>
      </div>

      {/* Attendance Modal (Same as before) */}
      {showAttendanceModal && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-fade-in-down">
            <div className="bg-[#3ac47d] p-6 flex justify-between items-center text-white">
              <div><h2 className="text-lg font-bold">Fee Transaction Ledger</h2><p className="text-xs text-white/80">{student.name} - Roll No: {student.rollNumber}</p></div>
              <button onClick={() => setShowFeeModal(false)} className="w-8 h-8 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center"><X size={18} /></button>
            </div>
            <div className="p-6">
              <div className="bg-green-50 border border-green-100 rounded-2xl p-4 flex justify-between items-center mb-6">
                 <span className="font-bold text-green-700 text-sm uppercase tracking-widest">Total Cleared</span>
                 <span className="font-black text-2xl text-[#3ac47d]">Rs {totalFeesPaid.toLocaleString()}</span>
              </div>
              <h3 className="font-bold text-sm text-slate-800 mb-3 border-b pb-2">Transaction History</h3>
              <div className="max-h-64 overflow-y-auto custom-scrollbar space-y-2 pr-2">
                {feeRecords.length === 0 ? (
                  <div className="flex flex-col items-center py-8 text-slate-400"><Receipt size={32} className="mb-2 opacity-50"/><p className="text-sm">No fee records found.</p></div>
                ) : (
                  feeRecords.map((r, i) => (
                    <div key={i} className="flex justify-between items-center p-3 bg-white border border-slate-100 rounded-xl hover:shadow-sm">
                      <div>
                        ...
                        <p className="text-[10px] font-medium text-slate-500">{new Date(r.createdAt?.toDate()).toLocaleDateString('en-GB')} • {r.month}</p>
                      </div>
                      <span className="font-black text-[#3ac47d]">Rs {r.amount}</span>
                    </div>
                  ))
                )}
              </div>
