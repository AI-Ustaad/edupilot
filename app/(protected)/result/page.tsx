    ...
    <div className="animate-fade-in space-y-6 pb-20">
      ...
      <div className="flex justify-between items-end print-hide">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-800 tracking-tight flex items-center gap-3">
            <Award className="text-primary"/> Exams & Grading
          </h1>
      ...
      <div className="glass-card p-6 print-hide">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <select value={selectedTerm} onChange={e => setSelectedTerm(e.target.value)} className="bg-white/60 backdrop-blur-sm outline-none rounded-xl px-4 py-3 text-sm border border-white/25 font-bold text-slate-800">
            {EXAM_TERMS.map(term => <option key={term} value={term}>{term}</option>)}
          </select>
          <select value={selectedClass} onChange={e => { setSelectedClass(e.target.value); setSelectedSection(""); }} className="bg-white/60 backdrop-blur-sm outline-none rounded-xl px-4 py-3 text-sm border border-white/25 font-bold text-slate-800">
            <option value="">-- Select Class --</option>
            {availableClasses.map(cls => <option key={cls as string} value={cls as string}>{cls as string}</option>)}
          </select>
          <select value={selectedSection} onChange={e => setSelectedSection(e.target.value)} disabled={!selectedClass} className="bg-white/60 backdrop-blur-sm outline-none rounded-xl px-4 py-3 text-sm border border-white/25 font-bold text-slate-800 disabled:opacity-50">
            <option value="">-- Select Section --</option>
            {availableSections.map(sec => <option key={sec.id} value={sec.sectionName}>{sec.sectionName}</option>)}
          </select>
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
            <input type="text" placeholder="Search student..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white/60 backdrop-blur-sm outline-none rounded-xl pl-9 pr-4 py-3 text-sm border border-white/25 font-bold text-slate-800" />
          </div>
        </div>
      </div>

      {/* STUDENTS RESULT GRID */}
      <div className="glass-card overflow-hidden min-h-[400px] print-hide">
        <div className="px-6 py-4 bg-slate-800 text-white flex items-center justify-between">
           <div>
             ...
             <p className="text-xs text-slate-300 font-medium">Generating results for: <span className="font-bold text-primary uppercase">{selectedTerm}</span></p>
           </div>
        </div>

        <div className="px-6 py-3 grid grid-cols-12 gap-4 bg-white/20 text-[10px] font-bold text-slate-600 uppercase tracking-widest">
        ...
        {(!selectedClass || !selectedSection) ? (
          ...
          <div className="divide-y divide-white/10">
            {filteredStudents.sort((a,b) => (a.rollNumber||0) - (b.rollNumber||0)).map((student) => {
              ...
              return (
                <div key={student.id} className="px-6 py-3 grid grid-cols-12 gap-4 items-center bg-white/10 hover:bg-white/20 transition-colors">
                  <div className="col-span-1 font-black text-slate-500 text-lg">{student.rollNumber || "-"}</div>

                  <div className="col-span-4 flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-white/60 border overflow-hidden shrink-0">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 uppercase">{student.classGrade} - {student.section}</p>
                    </div>
                  </div>
                  ...
                  <div className="col-span-2 flex justify-center items-center gap-2">
                     ...
                     {gradeStr !== "-" && <span className={`w-8 text-center py-1 rounded-md text-[10px] font-black ${gradeStr === "U" ? 'bg-red-100 text-red-600' : 'bg-success/20 text-success'}`}>{gradeStr}</span>}
                  </div>
                  ...
                  <div className="col-span-2 flex justify-end">
                     <button
                       ...
                       className="text-xs font-bold px-4 py-2 rounded-xl flex items-center gap-2 transition-colors disabled:opacity-30 disabled:bg-slate-100 disabled:text-slate-400 bg-primary/20 text-primary hover:bg-primary/30"
                     >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 print:p-0 print:bg-white overflow-y-auto">
           <div id="printable-result-card" className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden print:shadow-none print:max-w-full my-auto relative">
              <div className="absolute top-4 right-4 flex gap-2 print-hide z-50">
                 <button onClick={() => window.print()} className="bg-primary hover:bg-primary/80 text-white px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 shadow-md transition-colors">
              ...
              <div className="p-8 sm:p-12 bg-white">
                 <div className="text-center mb-8 border-b-[3px] border-slate-800 pb-6">
                    <div className="flex items-center justify-center gap-4 mb-4">
                       <div className="w-16 h-16 bg-slate-800 text-white rounded-2xl flex items-center justify-center shadow-lg transform -rotate-3">
                          <BookOpen size={36} />
                       </div>
                       <h1 className="text-4xl sm:text-5xl font-serif font-black text-slate-800 tracking-tighter">
                          EduPilot <span className="text-primary">Academy</span>
                       </h1>
                    </div>
                 ...
                 <div className="flex justify-between items-center mb-8 bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-sm">
                    <div className="flex items-center gap-5">
                       <div className="w-20 h-20 bg-white border-2 border-primary rounded-xl overflow-hidden p-1 shadow-sm">
                       ...
                       <div>
                          <h2 className="text-2xl font-black text-slate-800 leading-none">{selectedStudentForCard.name}</h2>
                          <p className="text-xs font-bold text-slate-500 uppercase mt-1.5">S/O: {selectedStudentForCard.fatherName}</p>
                       </div>
                    </div>
                    <div className="text-right">
                       <p className="font-black text-xl text-slate-800 uppercase">{selectedStudentForCard.classGrade} - {selectedStudentForCard.section}</p>
                       <div className="inline-block bg-slate-800 text-white px-4 py-1 rounded-lg mt-2">
                       ...
                       {(() => {
                         ...
                         return (
                           <>
                               ...
                               termMarks.map((m, idx) => {
                                 ...
                                 return (
                                   <tr key={idx} className="border-b border-slate-200">
                                      ...
                                      <td className="p-4 font-black text-slate-800 text-center border-r border-slate-200">{m.marksObtained}</td>
                             ...
                             {termMarks.length > 0 && (
                               <tr className="bg-green-50 border-t-2 border-primary">
                                  <td className="p-4 font-black text-slate-800 uppercase text-right border-r border-green-200">Grand Total:</td>
                                  <td className="p-4 font-black text-slate-800 text-center border-r border-green-200">{grandTotalMax}</td>
                                  <td className="p-4 font-black text-primary text-center text-xl border-r border-green-200">{grandTotalObt}</td>
                                  <td className="p-4 font-black text-slate-800 text-center border-r border-green-200">{grandTotalMax > 0 ? ((grandTotalObt/grandTotalMax)*100).toFixed(1) : 0}%</td>
                                  <td className="p-4 font-black text-white text-center bg-primary">
                   ...
                   if (termMarks.length > 0) {
                     ...
                     return (
                        <div className="mt-8 bg-blue-50/50 border border-blue-100 rounded-2xl p-6 relative overflow-hidden">
                           ...
                           <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest mb-3">Academic Guidelines & Progress</h3>
                 ...
                 <div className="flex justify-between items-end mt-20 pt-8 px-8">
                    <div className="text-center w-48"><div className="border-b-2 border-slate-800 pb-2 mb-2"></div><p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Class Incharge</p></div>
                    <div className="text-center w-48"><div className="border-b-2 border-slate-800 pb-2 mb-2"></div><p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Principal / Headmaster</p></div>
                 </div>
              </div>
           </div>
        </div>
