        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4 print:p-0 print:bg-white overflow-y-auto">
           <div id="printable-result-card" className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden print:shadow-none print:max-w-full my-auto relative">
              <div className="absolute top-4 right-4 flex gap-2 print-hide z-50">
                 ...
                 <button onClick={async () => { const element = document.getElementById("printable-result-card"); if (!element) return; const canvas = await html2canvas(element, { scale: 2, useCORS: true }); const imgData = canvas.toDataURL("image/png"); const pdf = new jsPDF("p", "mm", "a4"); const imgWidth = 210; const imgHeight = (canvas.height * imgWidth) / canvas.width; pdf.addImage(imgData, "PNG", 0, 0, imgWidth, imgHeight); pdf.save(`Result_${selectedStudentForCard.name || "Student"}.pdf`); }} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 shadow-md"><Printer size={14}/> Download PDF</button>
