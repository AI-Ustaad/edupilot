...
export default function ResultsPage() {
  ...
  const handleGeneratePDF = (studentId: string) => {
    // Opens the secure PDF generation endpoint in a new tab
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm flex flex-wrap gap-4">
        <div className="flex-1 min-w-[150px]">
          ...
          <select 
            ...
            className="w-full border border-gray-300 rounded-lg px-3 py-2 font-medium focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none"
          >
        ...
        <div className="flex-1 min-w-[150px]">
          ...
          <select 
            ...
            className="w-full border border-gray-300 rounded-lg px-3 py-2 font-medium disabled:bg-gray-100 disabled:text-gray-400 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none"
          >
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 bg-gray-50 border-b border-gray-200 font-bold text-gray-700 flex items-center justify-between">
          ...
          {results.length > 0 && (
            <span className="text-xs font-medium text-gray-500 bg-white px-3 py-1 rounded-full border border-gray-200 flex items-center gap-1">
              <Users size={14}/> {results.length} Students
            </span>
          )}
        </div>
        ...
        {isLoading ? (
          <div className="p-12 flex flex-col items-center justify-center text-gray-400">
            ...
            <p className="font-medium">Calculating results...</p>
          </div>
        ) : error ? (
          <div className="p-12 flex flex-col items-center justify-center text-red-500">
            <AlertCircle size={32} className="mb-3"/>
            <p className="font-medium">Failed to load results. Please check filters and try again.</p>
          </div>
        ) : results.length === 0 ? (
          <div className="p-12 flex flex-col items-center justify-center text-gray-400">
            <Search size={32} className="mb-3"/>
            <p className="font-medium">No results found for the selected criteria.</p>
            <p className="text-sm mt-1">Ensure marks have been entered for this class and term.</p>
          </div>
        ) : (
                  ...
                  <tr key={row.studentId} className="hover:bg-indigo-50/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold ${
                        row.grade === 'A++' || row.grade === 'A+' || row.grade === 'A' ? 'bg-green-100 text-green-700' :
                        row.grade === 'B' || row.grade === 'C' ? 'bg-blue-100 text-blue-700' :
                        row.grade === 'D' ? 'bg-yellow-100 text-yellow-700' :
