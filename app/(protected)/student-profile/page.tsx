...
function StudentProfileContent() {
  ...
  useEffect(() => {
    if (!studentId || !user?.tenantId) { router.push("/students"); return; }

    const fetchData = async () => {
      try {
        const docSnap = await getDoc(doc(db, "students", studentId));
        if (!docSnap.exists()) { router.push("/students"); return; }
        // ✅ as any to avoid TS error for tenantId
        const studentData = { id: docSnap.id, ...docSnap.data() } as any;
        ...
        const total = p + a + l;
        setAttendanceStats({ present: p, absent: a, leave: l, total, percentage: total > 0 ? Math.round((p / total) * 100) : 0 });
        setAttendanceRecords(aRecs.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()));

        // Fees with tenant filter
        const feeQ = query(collection(db, "fees"),
          where("studentId", "==", studentId),
          where("tenantId", "==", user.tenantId));
        ...
        setTotalFeesPaid(tFee);
        setFeeRecords(fRecs.sort((a, b) => b.createdAt?.toDate() - a.createdAt?.toDate()));

        // Marks with tenant filter
        const examQ = query(collection(db, "marks"),
          where("studentId", "==", studentId),
          where("tenantId", "==", user.tenantId));
    ...
    fetchData();
  }, [studentId, user?.tenantId, router]);
  ...
  return (
    <div className="animate-fade-in max-w-5xl mx-auto pb-20 relative">
      ...
      <div className="flex items-center justify-between mb-6 print:hidden">
        ...
        <button onClick={() => window.print()} className="flex items-center gap-2 bg-[#3ac47d] text-gray-900 px-5 py-2 rounded-lg font-bold text-sm shadow-md"><Printer size={16} /> Print Profile</button>
      </div>

      {/* Profile Banner */}
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden mb-8 relative">
        <div className="h-32 bg-[#3ac47d] relative overflow-hidden flex items-center justify-end px-10"><ShieldCheck size={100} className="text-gray-900/20 absolute -right-4 -bottom-4" /></div>
