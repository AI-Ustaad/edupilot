  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-8">
      <h1 className="text-3xl font-bold text-gray-900">
        {student.fullName || student.name} – Performance 360°
      </h1>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <p className="text-sm text-gray-500">Current Class</p>
          <p className="text-xl font-bold text-gray-900">{student.classGrade} {student.section || ""}</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <h2 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
          <TrendingUp className="text-blue-600" /> Attendance Trend (6 months)
        </h2>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <h2 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
          <BookOpen className="text-green-600" /> Marks Trend (by Term)
        </h2>
        {marksTrend.length > 0 ? (
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={marksTrend}>
              ...
              <YAxis domain={[0, 100]} stroke="#9ca3af" />
              <Tooltip />
              <Line type="monotone" dataKey="percentage" stroke="#10b981" strokeWidth={3} dot={{ r: 4 }} />
            </LineChart>
          </ResponsiveContainer>
        ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          <h2 className="text-lg font-bold text-gray-900 mb-4">Recent Quizzes</h2>
          {recentQuizzes.length === 0 ? (
            <p className="text-gray-500">No quizzes taken.</p>
          ) : (
            <ul className="space-y-2">
              {recentQuizzes.map((q: any) => (
                <li key={q.id} className="flex justify-between border-b pb-2">
                  <span className="text-gray-700">{q.quizTitle || "Quiz"}</span>
                  <span className="font-bold text-gray-900">{q.percentage}%</span>
                </li>
              ))}
            </ul>
          )}
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          ...
          {recentAssignments.length === 0 ? (
            <p className="text-gray-500">No assignments submitted.</p>
          ) : (
            <ul className="space-y-2">
              {recentAssignments.map((a: any) => (
                <li key={a.id} className="flex justify-between border-b pb-2">
                  <span className="text-gray-700">{a.fileName || "File"}</span>
                  <span className="text-sm text-gray-500">{new Date(a.createdAt?.toDate?.()).toLocaleDateString()}</span>
                </li>
              ))}
            </ul>
          )}
      ...
      {aiSuggestions && (
        <div className="bg-purple-50 border border-purple-200 rounded-xl p-6">
          ...
          <p className="text-gray-800 whitespace-pre-line">{aiSuggestions}</p>
        </div>
      )}
    </div>
  );
