    ...
    <RequirePermission permissions={["videoLectures.view" as any]}>
      <div className="p-6 max-w-6xl mx-auto">
        ...
        <div className="bg-white border border-gray-200 shadow-sm p-4 mb-6 flex flex-wrap gap-4 rounded-xl">
          <select value={filters.classGrade} onChange={e => setFilters({ ...filters, classGrade: e.target.value })} className="bg-gray-50 border border-gray-300 rounded-xl p-2 text-gray-900 flex-1 outline-none focus:ring-2 focus:ring-blue-500">
            <option value="">All Classes</option>
            {classes.map(c => (<option key={c}>{c}</option>))}
          </select>
          <select value={filters.subject} onChange={e => setFilters({ ...filters, subject: e.target.value })} className="bg-gray-50 border border-gray-300 rounded-xl p-2 text-gray-900 flex-1 outline-none focus:ring-2 focus:ring-blue-500">
