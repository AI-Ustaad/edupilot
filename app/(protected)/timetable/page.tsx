      ...
      <RequirePermission permissions={["timetable.create" as any]}>
        <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-8 shadow-sm">
          ...
          <form onSubmit={addEntry} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <input placeholder="Day" value={form.day} onChange={e => setForm({...form, day: e.target.value})} className="bg-gray-50 border border-gray-300 rounded-xl p-3 text-gray-900 focus:ring-2 focus:ring-blue-500 outline-none" required />
            <input placeholder="Period" value={form.period} onChange={e => setForm({...form, period: e.target.value})} className="bg-gray-50 border border-gray-300 rounded-xl p-3 text-gray-900 focus:ring-2 focus:ring-blue-500 outline-none" required />
            <input placeholder="Subject" value={form.subject} onChange={e => setForm({...form, subject: e.target.value})} className="bg-gray-50 border border-gray-300 rounded-xl p-3 text-gray-900 focus:ring-2 focus:ring-blue-500 outline-none" required />
            <input placeholder="Class" value={form.class} onChange={e => setForm({...form, class: e.target.value})} className="bg-gray-50 border border-gray-300 rounded-xl p-3 text-gray-900 focus:ring-2 focus:ring-blue-500 outline-none" required />
            <input placeholder="Teacher" value={form.teacher} onChange={e => setForm({...form, teacher: e.target.value})} className="bg-gray-50 border border-gray-300 rounded-xl p-3 text-gray-900 focus:ring-2 focus:ring-blue-500 outline-none" required />
              ...
              {entries.length === 0 ? <tr><td colSpan={6} className="p-6 text-center text-gray-400 font-medium">No entries yet.</td></tr> :
                entries.map((e: any) => (
                  <tr key={e.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-4 font-medium text-blue-600">{e.class}</td>
