...
import { useConfiguration } from "./ConfigurationProvider"; // 🟢 Rule 18: Get data from Context
import { useRuntimeStore } from "@/lib/store/runtime.store";

export function RuntimeProvider({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  const { config, isLoading, error } = useConfiguration(); // 🟢 Fetching via Context
  const initializeKernel = useRuntimeStore((state) => state.initializeKernel);

  // 🟢 Initialize Kernel when config is successfully loaded and Published
  useEffect(() => {
    // 🟢 FIX: Checking all required fields before booting
    if (config && config.state === "Published" && config.metadata && config.school && config.academic) {
      // 🟢 FIX: Using config.metadata.tenantId instead of config.tenantId
  ...
  // 3. For ALL other pages, enforce the Enterprise Shield & Diagnostics
  if (error || !config || config.state !== "Published") {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-red-50 p-6 text-center z-50 fixed inset-0">
        ...
        {/* 🚀 RULE 24: Runtime Diagnostics UI */}
...
// Helper Component for Diagnostics
