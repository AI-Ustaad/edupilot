        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex gap-2">
            <RequirePermission permissions={[PERMISSIONS.staff.create]}>
              ...
              <button onClick={() => setShowBulkModal(true)} className="bg-white border border-blue-600 text-blue-600 hover:bg-blue-50 px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 transition">
        ...
        {showBulkModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="bg-white w-full max-w-md mx-4 rounded-2xl shadow-xl p-6 relative">
              <button onClick={() => { setShowBulkModal(false); setBulkFile(null); setBulkMessage(null); }} className="absolute top-3 right-3 text-gray-400 hover:text-gray-700">
                <X size={20} />
              </button>
              <h2 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2"><Upload size={20} /> Bulk Import Staff</h2>
              <p className="text-sm text-gray-500 mb-4">Upload an Excel file (.xlsx) containing staff records.</p>
              <div className="mb-4">
                <input type="file" accept=".xlsx,.xls" onChange={(e) => { setBulkFile(e.target.files?.[0] || null); setBulkMessage(null); }} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
              </div>
              {bulkMessage && (
                <div className={`flex items-center gap-2 p-3 rounded-xl text-sm font-bold mb-3 ${bulkMessage.type === "success" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-600"}`}>
                  {bulkMessage.type === "success" ? <CheckCircle2 size={18} /> : <AlertTriangle size={18} />} {bulkMessage.text}
                </div>
              )}
              <div className="flex justify-end gap-3">
                <button onClick={() => { setShowBulkModal(false); setBulkFile(null); setBulkMessage(null); }} className="px-4 py-2 rounded-xl border border-gray-300 text-gray-700 font-bold hover:bg-gray-50 transition">Cancel</button>
                <button onClick={handleBulkUpload} disabled={uploading || !bulkFile} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl font-bold flex items-center gap-2 transition disabled:opacity-50">
                  {uploading ? <Loader2 className="animate-spin" size={16} /> : <Upload size={16} />} {uploading ? "Uploading..." : "Upload & Import"}
                </button>
              </div>
            </div>
          </div>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {staff.length === 0 ? (
                    ...
                    staff.map((s: any) => (
                      <tr key={s.id} className={`hover:bg-gray-50 transition ${deleteMutation.isPending && deleteMutation.variables === s.id ? 'opacity-50 pointer-events-none' : ''}`}>
                        ...
                        <td className="p-4 text-right">
                          <RequirePermission permissions={[PERMISSIONS.staff.delete]}>
                            <button onClick={() => handleDelete(s.id)} className="text-red-500 hover:bg-red-50 p-2 rounded-lg transition disabled:opacity-50" disabled={deleteMutation.isPending}>
                              {deleteMutation.isPending && deleteMutation.variables === s.id ? <Loader2 size={18} className="animate-spin"/> : <Trash2 size={18}/>}
                            </button>
                          </RequirePermission>
                        </td>
                      </tr>
