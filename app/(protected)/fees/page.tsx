    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm flex flex-wrap gap-4">
        ...
        <div className="flex-1 min-w-[150px]">
          ...
          <select value={selectedClass} onChange={e => setSelectedClass(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 font-medium">
        ...
        <div className="flex-1 min-w-[150px]">
          ...
          <select value={selectedSection} onChange={e => setSelectedSection(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 font-medium">
        ...
        {isLoading ? (
          ...
          <div className="divide-y divide-gray-100">
            {filteredStudents.map(student => {
              const existing = feeRecords.find((f: any) => f.studentId === student.id && f.month === selectedMonth);
              const currentAmount = feeEntries[student.id]?.amount || existing?.amount?.toString() || "0";
                ...
                <div key={student.id} className="px-6 py-4 flex items-center justify-between hover:bg-gray-50 transition">
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-400 uppercase">Class {student.class} - Section {student.section}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <input 
                      ...
                      placeholder="Amount"
                      value={currentAmount}
                      onChange={e => setFeeEntries(p => ({...p, [student.id]: {...p[student.id], amount: e.target.value, status: p[student.id]?.status || currentStatus}}))}
                      className="w-24 border border-gray-300 rounded-lg px-3 py-2 text-center font-bold"
                    />
                    <select 
                      value={currentStatus}
                      onChange={e => setFeeEntries(p => ({...p, [student.id]: {...p[student.id], status: e.target.value, amount: p[student.id]?.amount || currentAmount}}))}
                      className={`border rounded-lg px-3 py-2 font-bold ${currentStatus === 'paid' ? 'bg-green-100 border-green-300 text-green-700' : 'bg-orange-100 border-orange-300 text-orange-700'}`}
                    >
              ...
              <tbody className="divide-y divide-gray-100">
                {feeRecords.map((fee: any) => (
                  <tr key={fee.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-3 font-black text-gray-900">Rs. {fee.amount?.toLocaleString()}</td>
