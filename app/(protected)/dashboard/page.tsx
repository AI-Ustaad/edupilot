...
export default function DashboardPage() {
  ...
  const { data, isLoading: dataLoading, isError, error } = useQuery<DashboardData>({
    ...
    queryFn: async () => {
      // 🚀 Pointed to API v1 to maintain Data Contracts
      const res = await fetch("/api/v1/dashboard");
      if (!res.ok) throw new Error("Failed to fetch dashboard data");
      const json = await res.json();
      // Handle potential wrappers safely
    ...
    >
      ...
      <motion.div variants={fadeInUp}>
        <h1 className="text-3xl font-black text-gray-900">
          {branding.schoolName || t("title") || "Dashboard Overview"}
        </h1>
        <p className="text-gray-500 mt-1">{t("subtitle") || "Welcome back to your control center."}</p>
      </motion.div>
      ...
      >
        ...
        />
        <KpiCard
          title={t("todayAttendance") || "Attendance Today"}
          value={`${data.todayAttendance?.present || 0} / ${(data.todayAttendance?.present || 0) + (data.todayAttendance?.absent || 0)}`}
      ...
      {riskStudents.length > 0 && (
        ...
        >
          <h2 className="text-lg font-bold text-red-800 mb-4 flex items-center gap-2">
            <AlertTriangle className="text-red-600" /> {t("atRisk") || "At Risk Students"} ({riskStudents.length})
          </h2>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <motion.div variants={fadeInUp} className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
          <h3 className="text-lg font-semibold flex items-center gap-2 mb-4 text-gray-900">
            <CalendarDays size={20} style={{ color: primaryColor }} /> {t("weeklyAttendance") || "Weekly Attendance"}
          </h3>
        ...
        <motion.div variants={fadeInUp} className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
          <h3 className="text-lg font-semibold mb-4 flex items-center gap-2 text-gray-900">
            <CreditCard size={20} style={{ color: primaryColor }} /> {t("feeCollection") || "Fee Collection"}
          </h3>
          {data.feeMonth && (
            <div className="mb-4">
            ...
            {(data.classFeeSummary || []).map((c, idx) => (
              <div key={c.class || idx}>
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-gray-500">Rs {c.collected?.toLocaleString() || 0}</span>
                </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <motion.div variants={fadeInUp} className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
          <h3 className="text-lg font-semibold mb-4 flex items-center gap-2 text-gray-900">
            <Users size={20} style={{ color: primaryColor }} /> {t("studentDistribution") || "Student Distribution"}
          </h3>
          {!data.classDistribution || data.classDistribution.length === 0 ? (
            <div className="h-64 flex items-center justify-center text-gray-400">{t("noData") || "No Data Available"}</div>
          ) : (
              ...
              <tbody>
                {(data.recentPayments || []).map((p) => (
                  <tr key={p.id} className="border-b border-gray-100">
                    ...
                    <td className="p-3 text-green-600 font-bold">Rs {p.amount?.toLocaleString() || 0}</td>
                ...
                {(!data.recentPayments || data.recentPayments.length === 0) && (
                  <tr>
                    <td colSpan={4} className="p-6 text-center text-gray-400">{t("noData") || "No recent payments"}</td>
                  </tr>
                )}
              </tbody>
            </table>
