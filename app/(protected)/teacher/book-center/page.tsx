        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredBooks.map((book: any, idx: number) => (
              <div key={book.id || idx} className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition">
                <div className="w-12 h-12 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center mb-4">
                  <BookOpen size={24} />
                </div>
                <h3 className="font-bold text-lg text-gray-900 leading-tight">{book.title}</h3>
                <p className="text-sm text-gray-500 mt-1 font-medium">{book.classGrade} • {book.subject}</p>
                {book.fileUrl && (
                  <a href={book.fileUrl} target="_blank" rel="noopener noreferrer" className="mt-4 w-full bg-gray-50 hover:bg-gray-100 text-blue-600 font-bold py-2 rounded-xl flex items-center justify-center gap-2 transition border border-gray-200">
