...
export default function LoginForm() {
  // جیسے ہی پیج کھلے گا، یہ loading میں رہے گا تاکہ چیک کر سکے کہ یوزر گوگل سے تو نہیں آیا
  ...
  useEffect(() => {
    const checkGoogleReturn = async () => {
      try {
        // 1. چیک کریں کہ کیا یوزر ابھی گوگل سے لاگ ان ہو کر واپس آیا ہے؟
        ...
        if (result?.user) {
          ...
          // 3. سرور کو ٹوکن بھیجیں تاکہ کوکی (Cookie) بن سکے
          ...
          // 4. Smart Routing (ڈیش بورڈ یا سیٹ اپ پر بھیجیں)
          ...
          return; // اگر یوزر آ گیا ہے، تو loading کو true ہی رہنے دیں تاکہ فارم نظر نہ آئے
        }
      } catch (err: any) {
      ...
      // اگر یوزر گوگل سے واپس نہیں آیا (یعنی پیج پہلی بار کھلا ہے)، تو بٹن دکھا دیں
      setLoading(false);
      setStatusMsg("");
    };
