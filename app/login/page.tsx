...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // 2. فائر بیس کا نیا اور تازہ (Fresh) ٹوکن حاصل کریں
      const idToken = await result.user.getIdToken(true);

      // 3. ٹوکن کو اپنے بیک اینڈ پر بھیجیں تاکہ Secure Session Cookie بن سکے
      ...
      // 🔥 THE MAGIC FIX: یہاں router.push کی بجائے window.location.href لگائیں گے
      // یہ براؤزر کو زبردستی ریلوڈ کرے گا تاکہ مڈل ویئر (Middleware) نئی کوکی کو پڑھ لے!
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-[2rem] shadow-xl border border-slate-100 p-8 sm:p-10 text-center animate-fade-in-up">
        ...
        >
          {loading ? (
            <><Loader2 size={18} className="animate-spin text-blue-600" /> Routing to Command Centre...</>
          ) : (
            <>
              {/* Google G Logo SVG */}
