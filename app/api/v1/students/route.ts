...
export default function StudentsPage() {
  ...
  // 1. Bulletproof React Query Fetching
  const { data: studentsData, isLoading: isStudentsLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Safe Parsing: Empty رسپانس پر کریش ہونے سے بچانے کے لیے
      const text = await res.text();
      if (!text) return [];
      
      const json = JSON.parse(text);
      // 🔥 جادو: چیک کریں کہ ڈیٹا کہاں ہے، اگر Object ہے تو json.data نکالیں ورنہ خالی Array دیں
      return Array.isArray(json) ? json : (Array.isArray(json?.data) ? json.data : []);
    },
    enabled: !!user?.tenantId && !authLoading,
  });

  // Defensive Array Fallback (j.map ایرر کا حتمی علاج)
  ...
  const handleImportCSV = () => {
    // ... (Keep existing CSV import code as is)
                ...
                <tr key={s.id || idx} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-5 text-right">
                    <div className="flex justify-end gap-4 items-center">
                      {/* 🚀 Changed Link to point to the new 360 page */}
