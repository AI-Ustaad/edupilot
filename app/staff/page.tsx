...
export default function StaffPage() {
  ...
  const [documentBase64, setDocumentBase64] = useState<string>(""); // نیا: ڈاکومنٹ کے لیے
  ...
  const [formData, setFormData] = useState({
    ...
    allowances: [] as any[], deductionsList: [] as any[] // نیا: ڈیٹیل لسٹ کے لیے
  });
  ...
  const handleAIScan = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setFormData(prev => ({
        ...prev,
        ...data,
        cnic: data.cnic ? formatCNIC(data.cnic) : prev.cnic,
      }));
      alert("AI Scan Complete! All details extracted.");
    } catch (error) {
      setErrorMsg("AI Scan failed.");
    } finally {
      setIsScanning(false);
    }
  };
  ...
  const handleSaveStaff = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => window.location.reload(), 1500); // ریفریش کر دیں تاکہ فارم صاف ہو جائے
    } catch (error) {
